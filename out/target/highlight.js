"use strict";Object.defineProperty(exports, "__esModule", {value: true}); const hlcolors = {
    kw: '\x00r\x003',
    imm: '\x00r\x002',
    ident: '\x00r\x004',
    operator: '\x00r\x001',
    number: '\x00r\x005',
}; exports.hlcolors = hlcolors
...
const kwregex = new RegExp('(?<=\\s|^)(' + kw.join('|') + ')(?=\\s|$)', 'g')
 function highlight(k, hotrange = [0, 0]) {
    k = k
        ...
        .replaceAll('%n%', exports.hlcolors.number)
        .replaceAll('%i%', exports.hlcolors.ident)
        .replaceAll('%@%', exports.hlcolors.imm)
        .replaceAll('%k%', exports.hlcolors.kw)
        .replaceAll('%o%', exports.hlcolors.operator)
        .replaceAll('%r%', exports.nostyle)
        .replaceAll('%s%', '\x01!+')
        .replaceAll('%S%', '\x01!-')
    ...
    for (let i = 0; i < k.length; i++) {
        if (k[i] == '\x01' && k[i + 1] == '!') {
            ...
            i += 2
            mode = '3'
            if (is_str) output += exports.hlcolors.imm
            else output += exports.nostyle
            continue
        }
        if (k[i] == '\x00') {
        ...
        if (k[i] == '{' && k[i + 1] == '}' && is_str) {
            output += exports.hlcolors.ident + '{}' + exports.hlcolors.imm
            pos += 2
            i++
            continue
        }
 ...
 const ri = '\x005'; exports.ri = ri
 const opc = '\x00+\x002'; exports.opc = opc
 const cond = '\x003'; exports.cond = cond
 const label = '\x00r\x003'; exports.label = label
 const glob = '\x00+\x002'; exports.glob = glob
 const nostyle = '\x00r'; exports.nostyle = nostyle
 const comment = '\x00r\x000'; exports.comment = comment
 const fmt = {
    assign: '\x00a',
    cflow: '\x00b',
    unit: '\x00c',
...
} exports.finalizeColors = finalizeColors;
