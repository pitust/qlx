...
export function io(k: refkind): refclass { return { kind: k, mode: 'io' } }
...
export function resolveMatch(operations: opdef[], opkind: insn, opargs: ref[], deep: 'deep' | 'shallow') {
        ...
        for (let i = 0;i < aclass.length;i++) {
            ...
            if (targetc.mode == 'i') {
                ...
                try {
                    ...
                    al2.push(aether)
                    continue
                } catch (e) { if (e != 'shallow recursion prohibited!') throw e; }
...
export function printMatches(m: [string, ref[]][]) {
    for (const mm of m) {
        console.log('\x1b[31m%s\x1b[0m', mm[0], ...mm[1].map(e =>
            `\x1b[32m${e.kind.description}:\x1b[34m${e.type == 'real' ? '' : 'v'}${e.id}\x1b[0m`
        ))
    }
}
