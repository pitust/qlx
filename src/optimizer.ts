...
function eliminateDeadCode(blocks: SSABlock[]) {
    ...
    for (const match of findall(
        blocks,
        op => op.op == Opcode.StGlob || op.op == Opcode.StInitGlob
    )) {
    ...
    for (const match of findall(blocks, op => op.op == Opcode.StLoc || op.op == Opcode.StInitLoc)) {
        ...
        if (
            findall(blocks, op => {
                // fix a misoptimization in some structure cases
                if (op.op == Opcode.StLoc && op.args[0] == tgd) return false
                if (op.args.find(e => e && typeof e == 'object' && 'loc' in e && e.loc == tgd))
                    return true
                if (op.op == Opcode.LdLoc) return op.args[1] == tgd
                return false
            }).length
        )
            continue
...
function bindLoads(blocks: SSABlock[]) {
    ...
    for (const match of findall(
        blocks,
        op => op.op == Opcode.LdGlob || !!(op.op == Opcode.Move && glob(op.args[1]))
    )) {
        const r = reg(match.op.args[0])
...
function propagateConstants(blocks: SSABlock[]): boolean {
    ...
    for (const blk of blocks) {
        ...
        for (const op of blk.ops) {
            ...
            if (
                (op.op == Opcode.StGlob || op.op == Opcode.StInitGlob) &&
        ...
        if (blk.cond == JumpCond.TestBoolean && isKnown(blk.condargs[0])) {
            blk.cond = JumpCond.Always
            blk.targets = [blk.targets[+(getValue(blk.condargs[0]) == 0)]]
            blk.condargs = []
        }
        if (blk.cond == JumpCond.Equal && isKnown(blk.condargs[0]) && isKnown(blk.condargs[1])) {
            blk.cond = JumpCond.Always
            blk.targets = [blk.targets[+(getValue(blk.condargs[0]) != getValue(blk.condargs[1]))]]
            blk.condargs = []
        }
        if (blk.cond == JumpCond.TestBoolean && typeof blk.condargs[0] == 'number') {
            blk.cond = JumpCond.Always
                ...
                for (const blk of blkz) {
                    ...
                    for (const op of blk.ops) {
                        ...
                        if (op.op == Opcode.LdLoc) {
                            ...
                            if (isRoot) op.op = Opcode.LdGlob
                        }
                        if (op.op == Opcode.StLoc) {
                            op.args[0] = `${str(callop.args[1])}::${op.args[0]}`
                            if (isRoot) op.op = Opcode.StGlob
                        }
...
export function calculateCost(blocks: SSABlock[], count: number) {
    let cost = 0
    for (const blk of blocks) {
        for (const op of blk.ops) {
            if (op.op == Opcode.Call || op.op == Opcode.Asm) {
                cost += op.args.length
                continue
            }
